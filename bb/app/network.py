...
class Network(networkx.MultiDiGraph):
    ...
    class Edge(object):
        ...
        each node is represented by :class:`bb.app.mapping.Mapping`. Each edge
        has a direction (from `sender` to `receiver`, or back and forth), plus a
        set of attributes such as label (a text associated with it), etc.
