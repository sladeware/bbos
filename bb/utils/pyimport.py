...
# NOTE: Based on ihooks.py from python library:
#  <http://pydoc.org/2.4.1/ihooks.html>
#
# Read more about import hooks:
...
BUILTIN_MODULE = imp.C_BUILTIN
FROZEN_MODULE = imp.PY_FROZEN

class BasicModuleLoader(object):
  """Basic module loader.

  This provides the same functionality as built-in import. It doesn't deal with
  checking `sys.modules` -- all it provides is :func:`find_module` and a
  :func:`load_module`, as well as :func:`find_module_in_dir` which searches just
  one directory, and can be overridden by a derived class to change the module
  search algorithm when the basic dependency on sys.path is unchanged.

  The interface is a little more convenient than imp's: find_module(name,
  [path]) returns None or 'stuff', and load_module(name, stuff) loads the
  module.
  """
  ...
  def get_module_fullname(self, path):
    fullname = None
    for search_path in self.default_path():
      if path.startswith(search_path):
        mod_location = path[len(search_path) + 1:]
        parts = mod_location.split(os.sep)
        parts[-1] = parts[-1].split(".")[0] # remove suffix
        fullname = '.'.join(parts)
    return fullname
...
class Hooks(object):
  ...
  By deriving a subclass you can redefine your filesystem interface, e.g. to
  merge it with the URL space.
  ...
  def load_package(self, name, filename, file=None):
    return imp.load_module(name, file, filename, ("", "", imp.PKG_DIRECTORY))
...
class ModuleLoader(BasicModuleLoader):
  ...
  By defining suitable hooks, you might be able to load modules from other
  sources than the file system, e.g. from compressed or encrypted files, tar
  files or (if you're brave!) URLs.
  """
    ...
    if allow_packages:
      ...
      if self.hooks.path_isdir(fullname):
        ...
        if stuff:
          ...
          if file:
            file.close()
          return None, fullname, ("", "", imp.PKG_DIRECTORY)
  ...
  def load_module(self, name, stuff):
    ...
    if type == imp.PKG_DIRECTORY:
      ...
      if inittype not in (PY_COMPILED, PY_SOURCE):
        ...
        raise ImportError, \
            "Bad type (%r) for __init__ module in package %s" % (inittype, name)
      path = [filename]
      file = initfile
...
class BasicModuleImporter(object):
...
class ModuleImporter(BasicModuleImporter):
  ...
  def __init__(self, loader=None):
    BasicModuleImporter.__init__(self, loader)
  ...
  def determine_parent(self, globals, level=-1):
    ...
    try:
      ...
      if level < 1:
        warnings.warn("Parent module '%s' not found while handling absolute import" % pkgname, RuntimeWarning, 1)
        return None
      else:
  ...
  def ensure_fromlist(self, m, fromlist, recursive=0):
    for sub in fromlist:
      ...
      if sub != "*" and not hasattr(m, sub):
        ...
        if not submod:
          raise ImportError("No module 4named `%s' %s" % (subname, m))
...
def install_importer(importer=ModuleImporter()):
