...
def remove_tree(directory, dry_run=0):
...
def mkpath(name, mode=0777, verbose=0, dry_run=0):
  ...
  directory, which of course exists), then do nothing. Raise :class:`OSError` if
  unable to create some directory along the way (eg. some sub-path exists, but
  is a file rather than a directory). If 'verbose' is true, print a one-line
  summary of each mkdir to stdout. Return the list of directories actually
  created.
  """
  global _path_created
  # Detect a common bug -- name is None
  if not isinstance(name, types.StringTypes):
    raise types.TypeError("mkpath: 'name' must be a string (got %r)" % (name, ))
