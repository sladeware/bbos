...
def gen_main_c(kernel):
  ...
  if isinstance(kernel.get_scheduler(), StaticScheduler):
    ...
    for thread in kernel.get_threads():
      g.write('    %s();' % thread.get_runner())
    g.writeln('  }')
    g.writeln('}')
  # Init
  g.writeln('static void bbos_init()')
  g.writeln('{')
  #g.write('  BBOS_SET_NR_THREADS(%d);\n' % os.kernel.get_num_threads())
  #g.write('  BBOS_KERNEL_SET_RUNNING_TYPE(%d);\n' % 0)
  g.writeln('}')
  g.writeln()
  g.writeln('static void bbos_start()')
  g.writeln('{')
  #g.write('  bbos_kernel_start();\n')
  g.writeln('  bbos_loop();')
  g.writeln('}')
  g.writeln()
  g.writeln('void main%d(void* arg)' % kernel.get_core().get_id())
  g.writeln('{')
  g.writeln('  bbos_init();')
