...
/**
 * Deallocate memory chunk by its identifier.
 *
...
bbos_free(void *id)
{
  assert(BBOS_CHUNK_SIZE(id) < BBOS_MAX_ORDER);
  assert(ptsf_map[BBOS_CHUNK_SIZE(id)] != NULL);

  bbos_mempool_free(ptsf_map[BBOS_CHUNK_SIZE(id)], mem);
}

/**
 * Allocate memory chunk with specified size.
 *
 * @param sz Required memory chunk size.
 *
 * @return
 *
...
void
bbos_associate_pool(void *pool, uint16 sz)
{
  uint16 p;
