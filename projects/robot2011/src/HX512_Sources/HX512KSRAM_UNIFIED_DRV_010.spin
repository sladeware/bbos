{
HYDRA XTREME 512K SRAM CARD Driver (UNIFIED version)
...
Description: This is the "unified" driver that supports both ASM and SPIN versions of the drivers. By placing them in the same source
file it simply makes it easier to play with. The seperate drivers that are SPIN and ASM are named:
...
Use one of the above drivers if you want to save memory on your final program image. The spin driver is VERY slow and mostly for
tutorial purposes, the asm driver is literally hundreds of times faster, but obviously harder to follow, so if speed isn't a concern
you can start with the spin driver and then work to the asm driver. Both drivers have more or less the same functions, but
the spin driver functions end in "_S" and the asm driver functions end in "_A", so they can be in the same source file since spin
has no functional scoping.
