...
static struct serial_descriptor serial_descr; // serial descriptor
...
serial_send_byte(int byte)
{
  ...
  // wait if the head has looped right round and is now one less than the tail
  ...
  if (serial_descr.mode & SERIAL_MODE_IGNORE_TX_ECHO)
	echo = serial_receive_byte(); // why not check or timeout... this blocks for char
