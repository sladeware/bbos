...
from builder.fancy_getopt import FancyGetopt, translate_longopt
from builder.util import check_environ, strtobool, rfc822_escape
...
class Distributable:
    def __init__(self, extension):
        if not isinstance(extension, Extension):
            print "extension should based on Extension class"
            return
        self._extension = extension or Extension()

    def get_extension(self):
        return self._extension

class Distribution:
    """The core of the builder.  Most of the work hiding behind 'setup'
...
"""
    ...
    def find_config_files (self):
        ...
        There are three possible config files: builder.cfg in the
        Distutils installation directory (ie. where the top-level
        Distutils __inst__.py file lives), a file in the user's home
        directory named .pybuilder.cfg on Unix and pybuilder.cfg
        ...
        sys_file = os.path.join(sys_dir, "builder.cfg")
        if os.path.isfile(sys_file):
            files.append(sys_file)

        # What to call the per-user config file
        if os.name == 'posix':
            user_filename = ".pybuilder.cfg"
        else:
            user_filename = "pybuilder.cfg"
    ...
    def _show_help (self,
        ...
        """
        # late import because of mutual dependence between these modules
        from builder.core import gen_usage
        from builder.command import Command
    ...
    def print_commands (self):
        ...
        (listed in builder.commands.__all__) and "extra commands"
        (mentioned in self.cmdclass, but not a standard command).  The
        descriptions come from the command class attribute
        'description'.
        """

        import builder.commands
        std_commands = builder.commands.__all__
        is_std = {}
    ...
    def get_command_list (self):
        ...
        builder.commands.__all__) and "extra commands" (mentioned in
    ...
    def get_command_packages (self):
        ...
        if not isinstance(pkgs, type([])):
            ...
            if "builder.commands" not in pkgs:
                pkgs.insert(0, "builder.commands")
    ...
    def get_command_class (self, command):
        ...
        ("builder.command." + command) and fetch the command class from
...
class DistributionMetadata:
    ...
    def set_requires(self, value):
        import builder.versionpredicate
        for v in value:
            builder.versionpredicate.VersionPredicate(v)
        self.requires = value
    ...
    def set_provides(self, value):
        ...
        for v in value:
            import builder.versionpredicate
            builder.versionpredicate.split_provision(v)
        self.provides = value
